...
describe Superbolt::Messenger do
  describe 'where RabbitMQ is down' do
      it "rescues and notifies Airbrake if RabbitMQ connection fails" do
      messenger = Superbolt::Messenger.new(to: 'here')
      allow_any_instance_of(Superbolt::Adapter::Bunny).to receive(:establish_socket).and_raise(Bunny::TCPConnectionFailedForAllHosts)
      Airbrake.should_receive(:notify)
      messenger.send!('none').class.should eq (NilClass)
    end
  end

  describe 'where RabbitMQ is up' do
    let(:env) { 'test' }
